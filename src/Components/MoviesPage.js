...
import { baseUrl, fetchPopularMovies } from "../fetch";
        ...
        return (
          ...
          >
            ...
            <div className="absolute px-4 mt-auto flex justify-end py-4 flex-col h-full w-full hover:bg-gray-700 hover:bg-opacity-75">
              <div className="text-lg font-bold">{movie.original_title}</div>
              <div className="text-sm opacity-75">{movie.overview}</div>
              <div className="flex mt-3">
                <div className="text">{movie.release_date.split("-")[0]}</div>
      ...
      <div className="flex mt-8 mb-8 items-center justify-center">
        ...
        <select
          onChange={(e) => {
            ...
            fetch(
              `https://api.themoviedb.org/3/movie/${e.target.value}?api_key=e8ff140250eb7f3306d60b02a528008d&language=en-US&page=1&include_adult=false`
            )
              .then((res) => res.json())
              .then((resp) => {
                console.log(resp);
